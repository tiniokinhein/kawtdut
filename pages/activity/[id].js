...
const ActivityDetail = ({ item }) => {
    ...
    const { data:p } = useSWR(
        ACTIVITIES + `/${item.id}`,
        () => axios.get(ACTIVITIES + `/${item.id}`).then(res => res.data),
        {
    ...
    const { data:relates } = useSWR(
        ACTIVITIES + '?per_page=11',
        () => axios.get(ACTIVITIES + '?per_page=11').then(res => res.data),
        {
            refreshInterval: 1000
        }
    )
    ...
    const relateLists = relates && relates.filter(f => f.id !== (p && p.id)).length ? (
        <div className="col-12 col-md-4 mt-5 mt-md-0 ps-md-5">
            ...
            >
                ...
                </h4>
                {
                    ...
                    .map((m,i) => {
                        return(
                            <RelatedActivity key={i} relatedActivities={m} />
                        )
                    })
                }
            </div>
...
ActivityDetail.getInitialProps = (context) => {
