...
const CategoryDetail = ({ item }) => {
    ...
    const { data: posts  } = useSWR(
        NEWS + `?categories=${item.id}`,
        () => axios.get(NEWS + `?categories=${item.id}`).then(res => res.data),
        {
    ...
    const { data: p } = useSWR(
        CATEGORIES + `/${item.id}`,
        () => axios.get(CATEGORIES + `/${item.id}`).then(res => res.data),
        {
            refreshInterval: 1000
        }
    )
...
CategoryDetail.getInitialProps = (context) => {
